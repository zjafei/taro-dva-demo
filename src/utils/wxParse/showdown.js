...
function validate(extension, name) {
  ...
  for (let i = 0; i < extension.length; ++i) {
    ...
    if (type === 'listener') {
      ...
      ret.valid = false;
      ret.error = `${baseMsg +
        type} extensions must define either a "regex" property or a "filter" method`;
      return ret;
    }
      ...
      for (const ln in ext.listeners) {
        if (ext.listeners.hasOwnProperty(ln)) {
          if (typeof ext.listeners[ln] !== 'function') {
            ret.valid = false;
            ret.error = `${baseMsg}"listeners" property must be an hash of [event name]: [callback]. listeners.${ln} must be a function but ${typeof ext
              .listeners[ln]} given`;
            return ret;
          }
        }
...
showdown.Converter = function(converterOptions) {
  ...
  function _constructor() {
    ...
    if (typeof converterOptions === 'object') {
      ...
      throw Error(
        `Converter expects the passed parameter to be an object, but ${typeof converterOptions} was passed instead.`
      );
    }
  ...
  function _parseExtension(ext, name) {
    ...
    if (showdown.helper.isString(ext)) {
      ...
      if (showdown.extensions[ext]) {
        console.warn(
          `DEPRECATION WARNING: ${ext} is an old extension that uses a deprecated loading method.` +
...
showdown.subParser('headers', function(text, options, globals) {
  ...
  const headerLevelStart = isNaN(parseInt(options.headerLevelStart))
    ? 1
  ...
  function headerId(m) {
    let title;
    const escapedId = m.replace(/[^\w]/g, '').toLowerCase();
...
showdown.subParser('tables', function(text, options, globals) {
  ...
  text = text.replace(tableRgx, function(rawTable) {
    ...
    const rawStyles = tableLines[1].split('|').map(function(s) {
      return s.trim();
    });
    const rawCells = [];
    const headers = [];
