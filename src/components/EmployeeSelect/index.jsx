...
class CustomComponent extends Component {
  ...
  onReachBottom = () => {
    const {
      ...
      globalModel: {
        employeeIsLoading,
        employeeList: { data, totalItemCount },
        employeeListParams: { page },
      },
    } = this.props;
    if (employeeIsLoading === false && data.length < totalItemCount) {
      dispatch({
        type: 'globalModel/employeeChangeListParams',
        payload: {
          page: page + 1,
        },
      });
      ...
      return (
        <View key={item.id} className={styles.item} onClick={this.selectItem.bind(this, item)}>
          ...
          <View className={styles.itemCon}>
            ...
            <View className={styles.contact}>{item.phone}</View>
    ...
    return (
      <AtDrawer right show={show} width="320px" mask onClose={this.close}>
        <View className={styles.con}>
          ...
          >
            {employeeIsLoading !== null ? ( // 是否加载过
              data.length === 0 ? ( // 是否为空
                <Empty icon={getAssets('/assets/images/noAvatar.png')} title="还没有人员" />
              ) : (
                <View>
                  {list}
                  <NoMore visible={employeeIsLoading === false && data.length >= totalItemCount} />
                </View>
              )
            ) : null}
          </ScrollView>
